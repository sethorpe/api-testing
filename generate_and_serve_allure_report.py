...
def clean_allure_report(allure_report_path):
...
def generate_and_serve_allure_report():
    ...
    if not os.path.exists(allure_results) or not os.listdir(allure_results):
        print(f"No Allure results found in {allure_results}. Run your tests with --alluredir first.")
        return
    ...
    subprocess.run(["allure", "generate", allure_results, "--clean", "-o", allure_report], check=True)
